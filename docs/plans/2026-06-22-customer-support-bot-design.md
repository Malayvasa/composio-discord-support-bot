...
The bot should receive support requests in Discord, use Composio Sessions to access company support and diagnostics tools, consult repo-local runbooks, and return concise answers or escalation summaries.
...
- Answer common Composio questions from embedded runbooks.
...
- `src/support/runbooks.ts` loads markdown guidance from `knowledge/`.
- `src/discord/listeners.ts` decides when the bot should respond and builds Discord conversation context.
- `knowledge/` stores Composio and support debugging runbooks.
...
The bot's behavior is driven by runbooks, not just tool access. It should know core Composio concepts:
